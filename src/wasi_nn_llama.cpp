...
init_backend_with_config(void **ctx, const char *config, uint32_t config_len) {
  ...
  // Parse config JSON to update cleanup settings if provided
...
__attribute__((visibility("default"))) wasi_nn_error init_execution_context(
  ...
  LlamaChatContext *chat_ctx = (LlamaChatContext *)ctx;
  if (!chat_ctx || !chat_ctx->model || !session_id)
    return invalid_argument;
  ...
  *exec_ctx = new_exec_ctx;
  ...
  NN_INFO_PRINTF(
      "Execution context %d initialized for session '%s'. Active sessions: %zu",
      new_exec_ctx, session_id, chat_ctx->sessions.size());
