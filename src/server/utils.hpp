    ...
    std::vector<raw_buffer> &out_files)
{
    ...
    for (auto &msg : messages)
    {
        ...
        // Skip multimodal processing since we've removed mtmd support
        // This will just validate the content types
        for (auto &p : content)
        {
            std::string type = json_value(p, "type", std::string());
            if (type == "image_url" || type == "input_audio")
            {
                throw std::runtime_error("Multimodal inputs are not supported in this version");
            }
            else if (type != "text")
            {
 ...
 * Simplified version without multimodal support.
 */
struct server_tokens
{
    ...
    server_tokens(llama_tokens &tokens) : tokens(tokens) {}
